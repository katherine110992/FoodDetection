...
class FoodDetectorThread(Thread):
    ...
    def run(self):
        start_time = time()
        data_count = 0
        self.messages.append(self.name + " getting data from " + str(self.start_date)
                             + " to " + str(self.end_date) + "\n")
        data = self.semi_structured_access.get_from_database('conversation_current_period', "all_by_dates",
                                                             [self.start_date, self.end_date])
        self.messages.append(self.name + " total data to process: " + str(data.count()) + "\n")
        ...
        self.messages.append(self.name + " processed " + str(data_count)
                             + " from " + str(data.count()) + " data. Execution time: "
                             + str(timedelta(seconds=execution_time)) + "\n")
        print(self.name + " ended. Returning to main thread")
        return
