...
def simple_identification():
    ...
    p_file.write(date + " Detecting Emoticons & Emojis Test - Local Execution" + "\n")
    p_file.flush()
    # II. Prepare data
                ...
                if place is not None:
                    if "country_code" in place.keys():
                        ...
                        if raw_data_country_code in ["CO"]:
                            ...
                            text = raw_data['text']
                            if lang == 'es':
                                results = identify_special_characters(text, spanish_pipeline, tag_map, emoticons_dict,
                                ...
                                if len(text) >= 3:
                                    ...
                                    if detected_language == 'es':
                                        results = identify_special_characters(text, spanish_pipeline, tag_map,
                                                                              emoticons_dict,
...
def identify_special_characters(text, spanish_pipeline, tag_map, emoticons_dict, emojis_dict, emojis_list,
    ...
    for character in second_special_characters.keys():
        text = re.sub('(' + character + ')+', ' ' + character, text)
    # 3. Identify urls
    sp_counter, text, clean_text, sp_text = identify_first_special_characters(text, spanish_pipeline,
                                                                              first_special_characters)
    ...
    sp_counter, text, clean_text, sp_text, emoticons, emoticon_count = identify_emoticons(sp_counter, text, clean_text,
                                                                                          ...
                                                                                          emoticon_count)
    # 5. POS - Tokenize
    raw_tagged_text = spanish_pipeline(clean_text)
    sc_detected = False
    sc_text = ''
    final_clean_text = ''
    emojis = []
