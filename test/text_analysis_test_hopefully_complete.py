...
def complete_text_analysis():
                            ...
                            if lang == 'es':
                                ...
                                if len(original_text) >= 3:
                                    ...
                                    if detected_language == 'es':
                                        results = identify_special_characters(original_text, raw_entities,
                                                                              nlp,
                                                                              tag_map, emotions, emoticons_metadata,
                                                                              emotions_polarity, emojis_dict,
                                                                              emojis_list,
                                                                              variation_selectors,
                                                                              complementary_characters_dict,
                                                                              emoticon_count, emojis_count,
    ...
    p_file.write("Total data processed: " + str(count) + "\n")
    ...
    p_file.write("Total Emoticons: " + str(emoticon_count) + ". Proportion: " + str(emoticon_count / count) + "\n")
    ...
    p_file.write("Total Emojis: " + str(emojis_count) + ". Proportion: " + str(emojis_count / count) + "\n")
    ...
    p_file.write("Total Complementary Characters: " + str(complementary_characters_count)
                 + ". Proportion: " + str(complementary_characters_count / count) + "\n")
...
def identify_special_characters(text, raw_entities, nlp, tag_map, emotions, emoticons_metadata,
    ...
    if len(user_mentions) != 0:
        for user_mention in user_mentions:
            ...
            sp_name = 'special_entity_' + str(sp_id)
            ordered_special_entities.append(sp_name)
            token = original_text[indices[0]:indices[1]]
            simple_special_entities[sp_name] = token
            special_entities[sp_name] = {
                'token': token,
                        ...
                        if matches:
                            ...
                            for matchNum, match in enumerate(matches):
                                ...
                                if pattern_type != 'faces':
                                    if 'backwards' in pattern_type:
                                        last_character = sequence_detected[0]
                                        before_last_character = sequence_detected[1]
                                    else:
